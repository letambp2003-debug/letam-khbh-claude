...
export default function SignInButton() {
  ...
  return (
    <div style={{ maxWidth: 440, margin: '0 auto', textAlign: 'left' }}>
      <form onSubmit={handleCredentialsLogin} className="card" style={{ marginBottom: 16 }}>
        <label style={{ display: 'block', marginBottom: 8, fontWeight: 600, fontSize: 13.5 }}>
          Nhập Email giáo viên để truy cập:
        </label>
        <div style={{ display: 'flex', gap: 8 }}>
          <input
            type="text"
            placeholder="vd: letam@gmail.com"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            style={{ flex: 1 }}
          />
          <button type="submit" className="btn btn-primary" disabled={loading}>
            {loading ? 'Đang vào...' : 'Vào ngay 🚀'}
          </button>
        </div>
        <p className="muted" style={{ margin: '8px 0 0', fontSize: 12 }}>
          💡 Không cần tài khoản phức tạp. Bạn có thể nhập email bất kỳ hoặc bấm <strong>"Vào ngay 🚀"</strong> để trải nghiệm ngay lập tức.
        </p>
      </form>
